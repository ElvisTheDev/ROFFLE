...
/* Figure out which index is under the pointer at top */
function indexFromRotation(rotationDeg) {
  const rot = ((rotationDeg % 360) + 360) % 360;
  const target = (360 - rot) % 360; // pointer at 12 o'clock
    ...
    async function fetchPlayers() {
      ...
      try {
        ...
        const mapped = (data || []).map((row) => ({
          id: row.tg_id,
          name:
            row.full_name || row.username || (row.tg_id ? `User ${row.tg_id}` : "User"),
          ...
          {mode === "players" ? (
            ...
            <div className="lb-metric invites">
              <span className="lb-invites">
                {user.invites != null ? user.invites.toLocaleString() : "0"}
              </span>
...
export default function App() {
  ...
  const [referrals, setReferrals] = useState([]); // now from Supabase
  ...
  /* 🔹 NEW: Load referrals list from Supabase for Earn tab */
  useEffect(() => {
    if (!tgId) return;
  ...
  /* ===== Spin – client-side payout, visual index matched to prize ===== */
  const handleSpin = () => {
  ...
  // (Legacy ?ref= handler kept as-is, but now main invites come via bot)
  useEffect(() => {
    try {
  ...
  const shareLink = () => {
    ...
    if (tg?.openTelegramLink)
      ...
      navigator.share({ title: "ROFFLE", text, url: myRefLink }).catch(() => {});
    else
      window.open(
        `https://t.me/share/url?url=${encodeURIComponent(
          myRefLink
